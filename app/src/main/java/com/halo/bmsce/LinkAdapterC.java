...
public class LinkAdapterC extends FirestoreRecyclerAdapter<Link,LinkAdapterC.LinkHolder> {
    private OnItemClickListener listener;
    private String uri;
    public LinkAdapterC(@NonNull FirestoreRecyclerOptions<Link> options) {
        super(options);
    }
    ...
    public LinkHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View v= LayoutInflater.from(parent.getContext()).inflate(R.layout.contest_details_item,parent,false);
        return new LinkHolder(v);

    }

    class LinkHolder extends RecyclerView.ViewHolder{
        TextView text_attachment,text_link,workshop_date;
        ImageView image_attachment;

        public LinkHolder(@NonNull View itemView) {
            super(itemView);
            text_attachment=itemView.findViewById(R.id.text_attachmentc);
            text_link=itemView.findViewById(R.id.text_linkc);
            image_attachment=itemView.findViewById(R.id.image_attachmentc);
